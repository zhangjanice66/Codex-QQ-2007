...
async function main() {
  ...
  if (theme.decorations !== undefined && (!theme.decorations || typeof theme.decorations !== "object" || Array.isArray(theme.decorations))) {
    throw new Error("Theme decorations must be an object");
  }
  for (const [key, fileName] of Object.entries(theme.decorations || {})) {
    ...
    if (
      ...
      || fileName === "theme.json" || /[\u0000-\u001f\u007f-\u009f\u2028\u2029]/u.test(fileName)
    ) throw new Error(`Theme decoration ${key} must stay inside its theme directory`);
    decorationNames.push(fileName);
  }
  const assetNames = [...new Set([theme.image, ...decorationNames])];
  const assets = [];
  for (const fileName of assetNames) {
    const assetPath = path.resolve(sourceRoot, fileName);
    assertContained(sourceRoot, assetPath, "Theme asset");
    const asset = await readStableFile(assetPath, `Theme asset ${fileName}`, MAX_IMAGE_BYTES);
    if (asset.bytes.length < 1) throw new Error(`Theme asset ${fileName} is empty`);
  ...
  // Publish every referenced asset before theme.json, which remains the commit marker.
