...
if [ -d "$INSTALL_ROOT" ] && [ "$PROJECT_ROOT" != "$INSTALL_ROOT" ]; then
  ...
  for name in pause-dream-skin-macos.sh status-dream-skin-macos.sh apply-from-menubar-macos.sh \
    switch-theme-macos.sh load-image-theme-macos.sh install-menubar-macos.sh \
    personalize-codex-2007-macos.sh codex-2007-personalization.mjs \
    set-codex-1907-status-macos.sh replace-codex-1907-qq-show-macos.sh image-metadata.mjs; do
