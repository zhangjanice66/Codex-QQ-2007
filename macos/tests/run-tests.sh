...
/usr/bin/grep -F -q \
  'switch-theme-macos.sh" --id preset-codex-1907-deep --no-apply' \
...
/usr/bin/env HOME="$PERSONAL_HOME" NODE="$NODE" \
  ...
  --level 'LV09' \
  --status busy \
  --assistant "$ROOT/assets/portal-hero.png" \
...
"$NODE" -e '
  ...
  if (theme.profile.nickname !== "张奈斯" || theme.profile.signature !== "别迷恋姐，姐只是个传说。" ||
      theme.profile.level !== "LV09" || theme.profile.status !== "busy") process.exit(1);
  if (theme.tagline !== "别迷恋姐，姐只是个传说。" || theme.brandSubtitle !== "张奈斯 · 忙碌" ||
      theme.statusText !== "张奈斯 · 忙碌 · LV09") process.exit(1);
...
"$NODE" -e '
  const fs = require("node:fs");
  const theme = JSON.parse(fs.readFileSync(process.argv[1], "utf8"));
  if (theme.id !== "preset-codex-1907-deep" || theme.profile.nickname !== "张奈斯" ||
      theme.profile.signature !== "别迷恋姐，姐只是个传说。" || theme.profile.level !== "LV09" ||
...
"$NODE" -e '
  ...
  if (value.themeMode !== "deep" || value.profileNickname !== "张奈斯" ||
...
if /usr/bin/env HOME="$PERSONAL_HOME" NODE="$NODE" \
  ...
  --assistant "$ROOT/assets/portal-hero.png" --qq-show "$ROOT/assets/portal-hero.png" \
...
/usr/bin/env -u HOME /bin/bash -c '. "$1/scripts/common-macos.sh"; [ -n "$HOME" ] && [ "$SKIN_VERSION" = "1.7.17" ]' _ "$ROOT"
